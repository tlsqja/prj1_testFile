...
do 
	...
	case $order in 
		...
		5) 
			...
			echo
			if [ "$ans" = "y" ]; then 
				sed -n '1,10 s/^\([0-9]*\)|\([0-9]*\)|\([A-Z]\)|\([^|]*\)|.*/\1 \2 \3 \4/p' "$USER_File" | while IFS=" " read -r id age gender occupation; do
					gender="${gender/M/Male}"
					gender="${gender/F/Female}"

					echo "User $id is $age years old $gender $occupation"
				done
				echo
			fi
		...
		6)		
			...
			echo 
			if [ "$ans" = "y" ]; then
				#sed -n '1673,1682 s/\([0-9]\{2\}\)-\([A-Za-z]\{3\}\)-\([0-9]\{4\}\)/\3\2\1/p' "$ITEM_File"
				sed -n '1673,1682 {
